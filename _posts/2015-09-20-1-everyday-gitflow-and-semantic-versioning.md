...
tags: [ "GitFlow", "Semantic Versioning", "SemVer" ]
image:
...
How do we make sure that an application release never gets to production without passing the integration tests? How do we make sure that a class library [NuGet][NuGet] package never gets published to a public feed without passing the static code analysis requirements?  
<br/>
...
Before going into the details of what exactly we need to version, we have to think about how to do versioning at all. This topic is not new and, fortunately, we have Semantic Versioning, which is obviously versioned also conforming to itself as we can double-check reading through the [specifications][SemVer].  
<br/>
A nice extra in the .NET world is that NuGet itself [almost supports it][NuGetSemVer] as do [most of the public packages][Log4Net2]. At least we have an option here.

### GitFlow

For most of the projects, I have worked with GitFlow and always have felt it to be the first natural choice to go with. In case it turns out to be too complex, given the frequency of changes or the level of collaboration we can "fall back" to e.g., [GitHub Flow][GitHubFlow] where everything is "just" a hotfix.  
<br/>
I would like to add though that we don't even need Git for this setup, as the ALM Rangers also [describe][ARBranching]; it is just easier to implement with it than, for example, Subversion.  
<br/>
...
<br/>
For `AssemblyInformationalVersion`, we use exactly the same format as defined in the table above. This setup provides the version numbers as expected according to Semantic Versioning for the runtime itself, the CI build identification, and the resulting artifacts' package version.  
<br/>
Given some sample GitFlow-based steps, our NuGet packages will have the following versions after a CI build being triggered on commits:
...
<br/>
I have just came across that [GitVersion][GitVersion] supports these types of conventions natively in a highly customizable way, so this tool is definitely worth [taking a look at][Post2].
...
[Post2]: {% post_url 2015-10-30-2-gitversion-to-the-rescue %}
